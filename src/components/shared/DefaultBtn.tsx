...
const DefaultBtn = (props: { textBtn: string; methodBtn: string; equality?: boolean; disabled?: boolean }) => {
    ...
    const toggleClassBtn = (methodBtn: string) => {
        ...
        switch (methodBtn) {
            ...
            case 'SignIn':
                ...
                break;
            case 'CreatePassword':
                createPassword && confirmPassword && createPassword === confirmPassword ? changePasswordModal() : showErrorBtn('createPassword', createPassword, confirmPassword);
                break;
            case 'welcomeModal':
                ...
                break;
            case 'GetInfoModal':
                disableGetInfoBtn ? dispatch(getInfo('close-getInfo')) && dispatch(userLogIn('userFree')) : showErrorBtn('GetInfoModal', answerMessage, answer);
    ...
    return (
        <>
            <button className={`defaultBtn ${styleBtn}`} onClick={() => toggleClassBtn(props.methodBtn)} disabled={props.disabled}>
                {props.textBtn}
            </button>
        </>
    );
